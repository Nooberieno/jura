...
#define CurrentJuraVersion "5.0-standalone" 
...
void disableRawMode(){ //exit raw mode
...
int is_seperator(int c){ //check for seperate character
...
int SyntaxToColor(int hl){ //return the color that matches with the config
	switch(hl){
		case Comment:
		case MultiLineComment: return 36;
		case Keywords: return 33;
		case Types: return 34;
		case StringColor: return 31;
...
void FreeBuffer(struct buffer *buff){ //free the length for the character array
	free(buff->b); 
}
