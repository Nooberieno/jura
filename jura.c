  ...
  for (y = 0; y < config.screenlines; y++) {
    ...
    if(filerow >= config.numlines) {
      if(config.numlines == 0 && y == config.screenlines / 3) {
        char welcome[80];
        int welkom = snprintf(welcome, sizeof(welcome),
          "Jura  -- version %s", CurrentJuraVersion);
...
void DrawStatusBar(struct buffer *buff) {
  ...
  int len = snprintf(status, sizeof(status), "%.20s - %d lines %s",
    config.filename ? config.filename : "[No Name]", config.numlines,
    config.mod ? "(modified)" : "");
  int rlen = snprintf(rstatus, sizeof(rstatus), "%d/%d",
    config.y + 1, config.numlines);
  if(len > config.screencols) len = config.screencols;
...
void RefreshScreen() {
  Scroll();
  struct buffer buff = BufferStart;
    ...
    case PAGE_DOWN:
      {
        if(c == PAGE_UP) {
          ...
          if(config.y > config.numlines) config.y = config.numlines;
        }
        int times = config.screenlines;
        while (times--)
